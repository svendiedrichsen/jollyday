...
public class HolidayTest {
	...
	public void testIsHolidayPerformance() throws Exception {
		...
		LOG.log(Level.INFO, "isHoliday took " + sumDuration / count
				+ " millis average.");
	}
	...
	public void testChronology() throws Exception {
		GregorianChronology gregorianChronoUTC = GregorianChronology
				.getInstance(DateTimeZone.UTC);
		HolidayManager m = HolidayManager.getInstance("test");
		Set<Holiday> holidays = m.getHolidays(2010);
		for (Holiday d : holidays) {
			Assert.assertEquals("Wrong chronology.", gregorianChronoUTC, d
					.getDate().getChronology());
		}
	}
