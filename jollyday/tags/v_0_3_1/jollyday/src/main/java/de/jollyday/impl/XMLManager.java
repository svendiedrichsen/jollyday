...
public class XMLManager extends Manager {
	...
	public void init(String country) throws Exception {
		InputStream stream = getClass().getClassLoader().getResourceAsStream(FILE_PREFIX
				+ "_" + country + FILE_SUFFIX);
		if(stream == null){
			throw new IllegalArgumentException("Country "+country+" currently not supported. Please contact sdiedrichsen@yahoo.de to provide the data.");
		}
		try {
