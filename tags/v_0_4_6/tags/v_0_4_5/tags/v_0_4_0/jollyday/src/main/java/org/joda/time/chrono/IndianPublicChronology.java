...
/**
 * The indian public chronology represents the official indian calendar used by
 * the indian government. It is implemented like described in
...
public class IndianPublicChronology extends BasicChronology {
	...
	private static GregorianChronology GREGORIAN = GregorianChronology
			.getInstance();
	...
	private static final long MILLIS_YEAR_1 = new DateTime(79, 1, 1, 0, 0, 0,
			0, GREGORIAN).getMillis();

	private static final long MILLIS_IN_YEAR = 80L * DateTimeConstants.MILLIS_PER_DAY;

	/** A singleton era field. */
	private static final DateTimeField ERA_FIELD = new BasicSingleEraDateTimeField(
			"SAKA");
	...
	public static IndianPublicChronology getInstance(DateTimeZone zone) {
		...
		if (!cCache.containsKey(zone)) {
			IndianPublicChronology chrono = new IndianPublicChronology(null,
					null);
	...
	int getYear(long instant) {
		...
		long yearMillis = (isLeapYear(year) ? MILLIS_PER_LONG_YEAR
				: MILLIS_PER_SHORT_YEAR);
		while (cycleRemainder >= yearMillis) {
			cycleRemainder -= yearMillis;
			yearMillis = (isLeapYear(++year) ? MILLIS_PER_LONG_YEAR
					: MILLIS_PER_SHORT_YEAR);
		}
		return year;
	}
	...
	long calculateFirstDayOfYearMillis(int year) {
		...
		for (int i = 1; i < cycleRemainder; i++) {
			long yearMillis = isLeapYear(i) ? MILLIS_PER_LONG_YEAR
					: MILLIS_PER_SHORT_YEAR;
			millis += yearMillis;
		}
	...
	int getMonthOfYear(long millis, int year) {
		...
		do {
			month++;
			long value = ((long) getDaysInYearMonth(year, month))
	...
	long getTotalMillisByYearMonth(int year, int month) {
		...
		while (tempMonth < month) {
			millis += ((long) getDaysInYearMonth(year, tempMonth))
					* DateTimeConstants.MILLIS_PER_DAY;
			tempMonth++;
		}
		return millis;
